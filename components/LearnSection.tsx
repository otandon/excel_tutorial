...
function ConceptCard({ concept, index }: { concept: Module['concepts'][number]; index: number }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      {/* Card header with green left accent */}
      <div className="flex">
        <div className="w-1 bg-excel flex-shrink-0" />
        <div className="flex-1 p-6">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-7 h-7 rounded-full bg-excel-light flex items-center justify-center flex-shrink-0">
              <span className="text-excel text-xs font-bold">{index + 1}</span>
            </div>
          ...
          {concept.formula && (
            <div className="mt-4">
              ...
              <code className="block font-mono text-sm bg-gray-50 text-excel px-4 py-3 rounded-lg border border-gray-200 break-all">
                {concept.formula}
              </code>
            </div>
          )}
...
export default function LearnSection({ module, onStartQuiz }: LearnSectionProps) {
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      {module.id === 1 && (
        <div className="mb-6 bg-excel-light border-2 border-excel rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <a
            ...
            download="sample.csv"
            className="flex-shrink-0 bg-excel hover:bg-excel-dark text-white font-bold px-4 py-2 rounded-xl transition-colors duration-200 text-sm inline-flex items-center gap-2 whitespace-nowrap"
          >
            <span>⬇</span>
      ...
      <div className="flex items-center gap-2 mb-6">
        <span className="text-excel font-bold text-sm">📖 CONCEPTS</span>
      ...
      {/* CTA to quiz */}
      <div className="bg-excel-light rounded-2xl p-6 text-center">
        ...
        <button
          onClick={onStartQuiz}
          className="bg-excel hover:bg-excel-dark text-white font-bold px-8 py-3.5 rounded-xl transition-colors duration-200 text-base shadow-sm hover:shadow-md"
        >
          Start Quiz →
        </button>
      </div>
